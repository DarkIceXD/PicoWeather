...
static void command(const struct ili9341_display *display, const uint8_t command)
{
    ...
    spi_write_blocking(display->spi_port, &command, 1);
    gpio_put(display->cs, 1);
}

static void command_with_data(const struct ili9341_display *display, const uint8_t command, const uint8_t *data, const size_t len)
{
    gpio_put(display->cs, 0);
    gpio_put(display->dc, 0);
    spi_write_blocking(display->spi_port, &command, 1);
    gpio_put(display->dc, 1);
    spi_write_blocking(display->spi_port, data, len);
    gpio_put(display->cs, 1);
}
...
void ili9341_init(struct ili9341_display *display, spi_inst_t *spi_port, const uint reset, const uint dc, const uint cs)
{
    ...
    command(display, ILI9341_SWRESET);
    sleep_ms(5);
    command(display, ILI9341_DISPOFF);
...
{
    command_with_data(display, ILI9341_CASET, (uint8_t[]){x1 >> 8, (uint8_t)x1, x2 >> 8, (uint8_t)(x2)}, 4);
    command_with_data(display, ILI9341_PASET, (uint8_t[]){y1 >> 8, (uint8_t)y1, y2 >> 8, (uint8_t)(y2)}, 4);
}
